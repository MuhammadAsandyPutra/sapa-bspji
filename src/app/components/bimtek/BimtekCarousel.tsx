...
import BimtekCard from "./BimtekCard";
import type { Bimtek } from "@/types/bimtek";

interface BimtekCarouselProps {
    title?: string;
    items: Bimtek[];
}

export default function BimtekCarousel({
    title = "Upcoming Bimtek",
    items,
}: BimtekCarouselProps) {
    ...
    return (
        <section className="max-w-7xl mx-auto px-6 py-16">
            ...
            <div className="relative">
                ...
                >
                    {items.map((item) => (
                        <BimtekCard key={item.id} item={item} />
                    ))}
                </div>
